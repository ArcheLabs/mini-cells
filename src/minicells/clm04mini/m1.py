...
from .training import BaseCorpusDataset, BaseTrainConfig, train_base_model
...
def _tokenize_transaction(
    ...
    *,
    ...
    materialized = materialize_transaction(spec, smoke=smoke)
    return {
...
def run_m1_stream(
    *,
    ...
    """Execute registered primary variants from one identical base checkpoint."""
    ...
    for spec_index, spec in enumerate(specs):
        ...
        for variant_index, (variant, harness) in enumerate(harnesses.items()):
            ...
            if out_path is not None and (
                len(harness.records) % 16 == 0 or spec_index == len(specs) - 1
            ):
                harness.save_checkpoint(
                    out_path
                    / variant
                    / "checkpoints"
                    / f"tx-{spec.transaction_id:03d}.pt"
                )
...
def prepare_formal_data_assets(
    *,
    ...
    """Build tokenizer, 30M-token base shards, and the frozen curriculum manifest."""
...
def run_m1_infrastructure_smoke(
    *,
    ...
    """Exercise formal-scale interfaces without opening seed 90401/formal seeds."""
    protocol = load_protocol(protocol_path)
    ...
    for variant, harness in harnesses.items():
        ...
        actual = restored.summary()["final_state_hash"]
        checkpoint_replay[variant] = {"expected": expected, "restored": actual, "match": expected == actual}
