...
MINI Cells V0 implements deterministic Echo inference and one-generation-at-a-time SIGN-SPSA training as a Rust `no_std` MiniJAM service. Expensive neural evaluation runs in Refine. Accumulate only validates results, pairs PLUS/MINUS results, applies the bounded 4,476-parameter update, hashes the new model, and writes bounded state.
