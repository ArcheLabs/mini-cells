...
Run repository-owned checks with `./tools/test_all.sh`. For a running local MiniJAM stack, set `MINICELLS_RPC_URL`, `MINICELLS_KEEPER_SIGNER_URI`, and `MINICELLS_BULLETIN_DIR`, then use the direct `minicells` CLI or Keeper. Deploy with `minicells deploy service/artifacts/service.blob`, copy the canonical receipt-derived ID into `MINICELLS_SERVICE_ID`, then initialize via status probe, perform inference, advance one generation, and submit a stale training replay.
