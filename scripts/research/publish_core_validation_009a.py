#!/usr/bin/env python3
"""Publish Core Validation 009A phase artifacts and winner lock."""
...
RESULTS = ROOT / "results" / "core-validation-009a-factorized-functional-coordinates"
ARTIFACTS = ROOT / "artifacts" / "experiments" / "core-validation-009a-factorized-functional-coordinates"
...
def _copy_phase(phase: str) -> dict:
    ...
    if phase == "discovery" and decision.get("scientific_decision") is not False:
        raise RuntimeError("009A discovery must remain non-scientific")
    if phase == "confirmation":
        ...
        if not scientific and not partial:
            raise RuntimeError("confirmation publication requires final decision or CONFIRMATION_INCOMPLETE")
    ...
    if phase == "discovery":
        generated_lock = src / "winner-lock.json"
        if generated_lock.is_file():
            lock = json.loads(generated_lock.read_text(encoding="utf-8"))
            if lock.get("winner_meets_viability") is not True:
                raise RuntimeError("refusing to publish non-viable winner lock")
            shutil.copy2(generated_lock, WINNER_LOCK)
            shutil.copy2(generated_lock, ARTIFACTS / "winner-lock.json")
    elif WINNER_LOCK.is_file():
...
def _commit(phase: str, decision: dict) -> str | None:
    ...
    if WINNER_LOCK.is_file():
        paths.append(WINNER_LOCK.relative_to(ROOT).as_posix())
    ...
    if phase == "discovery":
        if decision.get("confirmation_allowed"):
            message = "research: lock Core Validation 009A factor split"
        else:
            message = f"research: checkpoint Core Validation 009A discovery ({len(decision.get('completed_seeds', []))}/2)"
    elif decision.get("scientific_decision") is True:
        message = "research: record Core Validation 009A confirmation"
    else:
        message = f"research: checkpoint Core Validation 009A confirmation ({len(decision.get('completed_seeds', []))}/3)"
...
def _push(branch: str, secret_name: str) -> None:
    ...
    print(f"pushed Core 009A artifacts to {branch}")
