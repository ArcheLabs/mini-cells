#!/usr/bin/env python3
"""Publish Core Validation 009B-2 phase artifacts and basis lock."""
...
EXCLUDE = {"frozen-hidden.pt", "effect-pack.pt"}
...
def _copy_phase(phase: str) -> dict:
    ...
    if phase == "discovery" and decision.get("scientific_decision") is not False:
        raise RuntimeError("009B-2 discovery must remain non-scientific")
    if phase == "confirmation":
        ...
        if not scientific and not partial:
            raise RuntimeError("009B-2 confirmation publication requires final decision or partial checkpoint")
    ...
    if phase == "discovery":
        generated = src / "basis-lock.json"
        if generated.is_file():
            lock = json.loads(generated.read_text(encoding="utf-8"))
            if lock.get("confirmation_allowed") is not True:
                raise RuntimeError("refusing to publish disabled 009B-2 basis lock")
            shutil.copy2(generated, BASIS_LOCK)
            shutil.copy2(generated, ARTIFACTS / "basis-lock.json")
    elif BASIS_LOCK.is_file():
        shutil.copy2(BASIS_LOCK, ARTIFACTS / "basis-lock.json")
    return decision
...
def _commit(phase: str, decision: dict) -> str | None:
    ...
    if BASIS_LOCK.is_file():
        paths.append(BASIS_LOCK.relative_to(ROOT).as_posix())
    run_git(ROOT, "add", "--", *paths)
    changed = run_git(ROOT, "diff", "--cached", "--quiet", check=False).returncode != 0
    if not changed:
        return None
    if phase == "discovery":
        message = "research: lock Core Validation 009B-2 effect dimension" if decision.get("confirmation_allowed") else f"research: checkpoint Core Validation 009B-2 discovery ({len(decision.get('completed_seeds', []))}/2)"
    elif decision.get("scientific_decision") is True:
        message = "research: record Core Validation 009B-2 persistent effect geometry"
    else:
        message = f"research: checkpoint Core Validation 009B-2 confirmation ({len(decision.get('completed_seeds', []))}/3)"
...
def _push(branch: str, secret_name: str) -> None:
    ...
    print(f"pushed Core 009B-2 artifacts to {branch}")
...
def main() -> int:
    ...
    if args.preflight_only:
        preflight_push(branch, args.secret_name); return 0
