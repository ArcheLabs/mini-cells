#!/usr/bin/env python3
"""Run one frozen Core Validation 009A phase with per-seed checkpoints."""
...
def _hydrate(phase: str, seed: int) -> bool:
    ...
    print(f"[core009a] hydrated published {phase} seed {seed}", flush=True)
    return True


def _report(phase: str) -> dict:
    _run([sys.executable, "scripts/research/report_core_validation_009a.py", "--phase", phase])
...
def _publish(phase: str, branch: str, secret_name: str) -> None:
    _run([
        sys.executable,
        "scripts/research/publish_core_validation_009a.py",
        "--phase", phase,
        "--push-results",
        "--branch", branch,
...
def main() -> int:
    ...
    p.add_argument("--branch", default="codex/core-validation-009a-factorized-functional-coordinates")
    ...
    if args.phase == "confirmation" and not WINNER_LOCK.is_file():
        raise RuntimeError("confirmation requires committed winner-lock.json; run and publish discovery first, then use a fresh/pulled checkout")
    ...
    for seed in seeds:
        ...
        if complete:
            print(f"[core009a] {args.phase} seed={seed} complete; skipping", flush=True)
        else:
            _run([
                sys.executable,
                "scripts/research/run_core_validation_009a_seed.py",
                "--phase", args.phase,
                "--seed", str(seed),
                "--device", args.device,
            ])
    ...
    if args.phase == "discovery":
        if decision.get("confirmation_allowed"):
            print("[core009a] discovery complete and viable. Winner lock was published; start confirmation only from a checkout containing that lock.", flush=True)
        else:
            print("[core009a] discovery is not viable; protocol stop rule forbids confirmation.", flush=True)
    return 0
