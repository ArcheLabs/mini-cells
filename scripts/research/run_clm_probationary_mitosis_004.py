#!/usr/bin/env python3
"""Launch and aggregate the formal CLM-0.3d probationary-mitosis replicates."""
...
def _worker_dir(root: Path, replicate: int) -> Path:
    return root / f"r{replicate}-probationary"
...
def _complete(directory: Path, code_commit: str) -> bool:
    ...
    try:
        ...
        return False
    return (
        value.get("code_commit") == code_commit
        and int(value.get("births_checked", 0)) == 24
        and int(value.get("births_equivalent", 0)) == 24
    )
...
def _prepare_corpora(args: argparse.Namespace) -> None:
    ...
    train_tokens = max(
        args.decision_tokens + FORMAL_HORIZONS[-1] + SEQUENCE_LENGTH + 2,
        int(manifest["train_stream_tokens"]),
    )
    validation_tokens = max(100_000, int(manifest["validation_stream_tokens"]))
    print(
        f"Preparing CLM-0.3d shared corpora: story train={train_tokens:,} "
    ...
    print(
        ...
        f"story_train_sha={produced.get('train_token_sha256')} "
        f"story_validation_sha={produced.get('validation_token_sha256')} "
        f"arithmetic_train_sha={arithmetic['manifest'].get('train_sha256')}",
        flush=True,
    )
...
def _dashboard(directory: Path, replicate: int, gpu: int) -> str:
    ...
    try:
        ...
        return (
            ...
            f"| {phase:<34} | {throughput/1000:.1f}K tok/s"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run formal CLM-0.3d probationary mitosis")
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("results/clm-0.3d-probationary-mitosis"),
    )
    parser.add_argument(
        "--release-dir",
        type=Path,
        default=Path("artifacts/releases/clm-0.1"),
    )
    parser.add_argument(
        "--source-005-dir",
        type=Path,
    ...
    parser.add_argument("--cache-dir", type=Path, default=Path("results/.clm-0.3d-cache"))
    ...
    if args.execute and provenance["tracked_tree_dirty"]:
        raise RuntimeError("formal CLM-0.3d matrix requires a clean tracked Git tree")
    if args.execute and args.decision_tokens != DECISION_TOKENS:
        raise RuntimeError(
            "formal CLM-0.3d decision checkpoint is preregistered at 1.5M tokens; "
            "custom horizons require a separate non-formal experiment"
        )
    ...
    if not args.execute:
        ...
        print(
            "Each replicate: 1.5M shared trunk; then stationary_story and "
            "story_arithmetic_shift. Per condition: 500K no-growth control + "
            "12×100K shadows + 4×400K shortlisted continuation.",
            flush=True,
        )
        print(
            f"Probation ages={FORMAL_HORIZONS}; shortlist K={SHORTLIST_K}; "
            "holdout B is never used for selection.",
            flush=True,
        )
        print("PREFLIGHT ONLY: pass --execute to launch CLM-0.3d.", flush=True)
        return 0

    gpu_count = torch.cuda.device_count()
    if gpu_count < 1:
        raise RuntimeError("formal CLM-0.3d execution requires CUDA")
    _prepare_corpora(args)
    ...
    for replicate in range(3):
        ...
        if args.restart_existing and directory.exists():
            print(f"RESTART r{replicate}: removing prior 0.3d evidence", flush=True)
            ...
            raise RuntimeError(
                ...
                f"{code_commit[:12]}; use --restart-existing rather than mixing commits"
            )
    ...
    while pending or active:
        while pending and len(active) < capacity:
            ...
            env["PYTHONPATH"] = (
                str(RESEARCH_ROOT)
                if not current
                else str(RESEARCH_ROOT) + os.pathsep + current
            )
        ...
        if active:
            lines = [
                _dashboard(_worker_dir(args.output_root, rep), rep, gpu)
                for rep, gpu, _ in active
            ]
            print(
                "DASHBOARD | "
                + " || ".join(lines)
                + f" | completed {completed}/3 | pending {len(pending)}",
                flush=True,
            )
        time.sleep(2)
    ...
    result = aggregate_probationary_results(
        args.output_root,
