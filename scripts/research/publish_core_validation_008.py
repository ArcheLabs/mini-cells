#!/usr/bin/env python3
"""Publish Core Validation 008 artifacts while excluding the hidden-state cache."""
...
def _copy_results() -> None:
    ...
    if decision.get("experiment_id") != "core-validation-008":
...
def _commit() -> str | None:
    ...
    if not changed:
        return None
    run_git(ROOT, "commit", "-m", "research: record Core Validation 008 certified functional atoms")
...
def _push(branch: str, secret_name: str) -> None:
    ...
    print(f"pushed Core 008 results to {branch}")
