"""Publish Native CLM v0 M3R checkpoints to Hugging Face and lightweight evidence to Git."""
...
DEFAULT_BRANCH = "codex/native-clm-v0-m3r-read-preserving-growth"
...
def _checkpoint_records(output_dir: Path, formal_seeds: list[int]) -> list[dict]:
    ...
    for seed in formal_seeds:
        for arm in ("global_growth_control", "lineage_growth"):
            ...
            records.append(
                {
                    ...
                    "path": f"m3r/seed-{seed}/{arm}-final.pt",
...
def _publish_hf(
    *,
    ...
    result = {
        "format": "minicells.native-clm-v0.m3r-model-artifacts.v1",
        "repo_id": repo_id,
    ...
    try:
        for record in records:
            ...
            print(
                f"Uploading M3R seed={record['seed']} arm={record['arm']} -> "
            ...
            api.upload_file(
                ...
                repo_id=repo_id,
                repo_type="model",
                commit_message=f"Native CLM v0 M3R seed {record['seed']} {record['arm']}",
            )
            record["uploaded"] = True
        api.upload_file(
            path_or_fileobj=str(decision_path),
            path_in_repo="m3r/decision.json",
            repo_id=repo_id,
            repo_type="model",
            commit_message="Native CLM v0 M3R decision",
        )
        ...
        if require_upload:
            raise RuntimeError("Hugging Face M3R checkpoint upload failed") from exc
    return result
...
def main() -> int:
    ...
    if decision.get("format") != "minicells.native-clm-v0.m3r-decision.v1":
        raise RuntimeError("unexpected M3R decision format")
    ...
    if not allowed:
        raise RuntimeError("no lightweight M3R artifacts found")
    run(["git", "reset"])
    ...
    if staged:
        ...
        run(["git", "commit", "-m", "research: publish Native CLM v0 M3R results"])

    refspec = f"+refs/heads/{args.branch}:refs/remotes/origin/{args.branch}"
    print("Fetching only the M3R research branch...", flush=True)
    run(["git", "fetch", "--no-tags", "origin", refspec], timeout=180)
    print("Rebasing M3R evidence commit...", flush=True)
    try:
    ...
    askpass = Path("/tmp/minicells-m3r-askpass.sh")
    askpass.write_text(
    ...
    try:
        print("Pushing lightweight M3R evidence...", flush=True)
        run(
            [
                "git",
                "push",
