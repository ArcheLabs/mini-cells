#!/usr/bin/env python3
"""Publish Core Validation 009C checkpoints and representation lock."""
...
def _copy_phase(phase: str) -> dict:
    ...
    if phase == "discovery" and decision.get("scientific_decision") is not False:
        raise RuntimeError("009C discovery must remain non-scientific")
    if phase == "confirmation":
        ...
        if not scientific and not partial:
            raise RuntimeError("009C confirmation publication requires final decision or partial checkpoint")
    ...
    if phase == "discovery":
        ...
        if generated.is_file():
            ...
            if lock.get("confirmation_allowed") is not True:
                raise RuntimeError("refusing to publish disabled 009C representation lock")
            shutil.copy2(generated, LOCK)
            shutil.copy2(generated, ARTIFACTS / "representation-lock.json")
    elif LOCK.is_file():
        shutil.copy2(LOCK, ARTIFACTS / "representation-lock.json")
    return decision
...
def _commit(phase: str, decision: dict) -> str | None:
    ...
    if phase == "discovery":
        if decision.get("confirmation_allowed"):
            message = "research: lock Core Validation 009C representation"
        else:
            message = f"research: checkpoint Core Validation 009C discovery ({len(decision.get('completed_seeds', []))}/2)"
    elif decision.get("scientific_decision") is True:
        message = "research: record Core Validation 009C confirmation"
    else:
        message = f"research: checkpoint Core Validation 009C confirmation ({len(decision.get('completed_seeds', []))}/3)"
...
def _push(branch: str, secret_name: str) -> None:
    ...
    print(f"pushed Core 009C artifacts to {branch}")
