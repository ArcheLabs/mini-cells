#!/usr/bin/env python3
"""Run one frozen Core Validation 009D discovery or confirmation seed."""
...
from minicells.real_representation_009d_experiment import run_geometry
...
PARENT_009C = ROOT / "artifacts" / "experiments" / "core-validation-009c-sparse-local-effect-geometry" / "discovery" / "decision.json"
PARENT_009A = ROOT / "artifacts" / "experiments" / "core-validation-009a-factorized-functional-coordinates" / "confirmation" / "decision.json"
DEFAULT_OUT = ROOT / "results" / "core-validation-009d-compositional-operator-geometry"
...
def _heartbeat(seed: int, message: str) -> None:
    print(f"[core009d seed={seed}] {message}", flush=True)


def _validate_parents(protocol: dict[str, Any]) -> None:
    if not PARENT_009C.is_file() or not PARENT_009A.is_file():
        raise RuntimeError("009D requires published 009C discovery and 009A confirmation decisions")
    d009c = json.loads(PARENT_009C.read_text(encoding="utf-8"))
    d009a = json.loads(PARENT_009A.read_text(encoding="utf-8"))
    parent = protocol["parent_evidence"]
    if d009c.get("status") != parent["core009c_status"]:
        raise RuntimeError("009C parent status mismatch")
    if d009c.get("protocol_sha256") != parent["core009c_protocol_sha256"]:
        raise RuntimeError("009C parent protocol hash mismatch")
    if d009c.get("confirmation_allowed") is not False:
        raise RuntimeError("009D requires frozen 009C discovery failure")
    if d009a.get("status") != parent["core009a_status"] or d009a.get("supported") is not True:
        raise RuntimeError("009A positive factor geometry is required by 009D")
    if d009a.get("protocol_sha256") != parent["core009a_protocol_sha256"]:
        raise RuntimeError("009A parent protocol hash mismatch")
    split = d009a.get("locked_split") or {}
    if int(split.get("left_dim", -1)) != int(parent["core009a_locked_left_dim"]) or int(split.get("right_dim", -1)) != int(parent["core009a_locked_right_dim"]):
        raise RuntimeError("009A locked factor split mismatch")
...
def _load_lock() -> dict[str, Any]:
    ...
    if lock.get("format") != "minicells.core-validation.compositional-operator-geometry-lock.v1":
        raise RuntimeError("invalid 009D representation lock format")
    if lock.get("protocol_sha256") != _sha256(PROTOCOL):
        raise RuntimeError("009D representation lock protocol hash mismatch")
    if lock.get("confirmation_allowed") is not True:
        raise RuntimeError("009D confirmation forbidden by representation lock")
    return lock
...
def main() -> int:
    ...
    _heartbeat(args.seed, "extracting full normalized write operators")
    ...
    if not causal or any(s.partition not in {"train", "eval"} for s in causal):
        raise RuntimeError("009D train/eval filtering failed")
    _heartbeat(args.seed, f"operators ready: train={sum(s.partition == 'train' for s in causal)} eval={sum(s.partition == 'eval' for s in causal)}")
