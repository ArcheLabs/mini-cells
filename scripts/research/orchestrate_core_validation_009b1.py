#!/usr/bin/env python3
"""Run one frozen Core Validation 009B-1 phase with per-seed checkpoints."""
...
def _valid(path: Path, phase: str, seed: int) -> bool:
    ...
    try:
        ...
        return False
    return (
        payload.get("format") == _format(phase)
...
def _hydrate(phase: str, seed: int) -> bool:
    ...
    print(f"[core009b1] hydrated published {phase} seed {seed}", flush=True)
    return True


def _report(phase: str) -> dict:
    _run([sys.executable, "scripts/research/report_core_validation_009b1.py", "--phase", phase])
...
def _publish(phase: str, branch: str, secret_name: str) -> None:
    _run([
        sys.executable,
        "scripts/research/publish_core_validation_009b1.py",
        "--phase", phase,
        "--push-results",
        "--branch", branch,
...
def main() -> int:
    ...
    p.add_argument("--branch", default="codex/core-validation-009b1-carrier-causal-sufficiency")
    ...
    if args.phase == "confirmation" and not SCALE_LOCK.is_file():
        raise RuntimeError(
            "confirmation requires committed scale-lock.json; run and publish discovery first, then refresh the checkout"
        )
    ...
    for seed in seeds:
        ...
        if complete:
            print(f"[core009b1] {args.phase} seed={seed} complete; skipping", flush=True)
        else:
            _run([
                sys.executable,
                "scripts/research/run_core_validation_009b1_seed.py",
                "--phase", args.phase,
                "--seed", str(seed),
                "--device", args.device,
            ])
    ...
    if args.phase == "discovery":
        if decision.get("confirmation_allowed"):
            print(
                "[core009b1] discovery complete. Scale lock was published; run confirmation only from a refreshed checkout containing scale-lock.json.",
                flush=True,
            )
        else:
            print(
                "[core009b1] no frozen causal scale is viable; stop rule forbids confirmation.",
                flush=True,
            )
    return 0
