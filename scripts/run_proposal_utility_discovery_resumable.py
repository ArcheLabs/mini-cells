...
def _standardize_finite(
    ...
    # Very large z-scores can destabilize the tiny diagnostic MLP without
    # adding useful information to this signal-discovery experiment.
...
def _postprocess(gpu_count: int) -> dict[str, object]:
    ...
    print("finite audit:")
    print(audit.loc[audit["nonfinite_rows"] > 0].to_string(index=False) or "all finite")
    _validate_oracles(observations)

    # Patch only the estimator-side numerical helpers. The scientific oracle,
    # folds, features, thresholds, and pre-registered decision logic stay
    # unchanged.
