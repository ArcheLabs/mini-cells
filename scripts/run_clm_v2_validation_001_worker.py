...
from minicells.clm_v2_training import (  # noqa: E402
    HANDOFF_STAGES, K_STAGES, handoff_loss, latest_stage_checkpoint,
    normalized_local_loss, save_v2_checkpoint,
)
from minicells.clm_v2_validation import (  # noqa: E402
    V2RoutingRecorder, evaluate_v2, evaluation_dict, static_mask,
    v2_router_diagnostics, validate_v2_scaffold_parity,
)
...
from minicells.language_data import (  # noqa: E402
    batch_from_starts, fixed_validation_starts, make_training_schedule,
)
from minicells.textnca_to_clm_v2 import convert_textnca_to_clm_v2  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run one CLM v2 scaffold-handoff replicate.")
...
def evaluate_current(model, stream, starts_batches, device) -> dict[str, float]:
    ...
    return {"nll": nll, "ppl": math.exp(min(nll, 20)),
            "local_relative_mse": relative / count, "local_cosine": cosine / count}
...
def train_tokens(
    ...
    for starts in schedule.starts:
        ...
        grad_norms.append(float(torch.cat(router_gradients).norm()))
    ...
    return {
        "loss": losses[-1], "local_relative_mse": relative[-1],
        "router_grad_norm": sum(grad_norms) / len(grad_norms),
        "elapsed_seconds": time.perf_counter() - started,
    }


def checkpoint_payload(student, optimizer, scheduler, scaler, replicate, stage_index,
                       rows, diagnostics, status="RUNNING"):
    return {
        "format": "minicells.clm-v2-validation-001-checkpoint.v1",
        "replicate": replicate, "stage_index": stage_index, "status": status,
        ...
        "rows": rows, "diagnostics": diagnostics, "rng_state": torch.random.get_rng_state(),
        "cuda_rng_state": torch.cuda.get_rng_state_all(),
        "scaffold_provenance": student.scaffold_provenance,
    }


def final_controls(student, validation, calibration_starts, formal_starts,
    ...
    shuffled = [evaluate_v2(
        ...
        permutation_seed=102001 + replicate * 100 + index,
    ...
    for key in ("nll", "sample_variation", "position_variation", "temporal_variation",
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CLM v2 validation requires CUDA")
    ...
    gate_starts = fixed_validation_starts(
        validation.numel(), batches=16, batch_size=8, sequence_length=128, seed=6105
    )
    calibration_starts = fixed_validation_starts(
        validation.numel(), batches=8, batch_size=8, sequence_length=128, seed=6205
    )
    formal_starts = fixed_validation_starts(
        validation.numel(), batches=24, batch_size=8, sequence_length=128, seed=6305
    )
    teacher = load_experiment_006_teacher(
        str(args.checkpoint), device=device, model_config_path=str(args.model_config)
    )
    seed = 93001 + args.replicate
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
    student = convert_textnca_to_clm_v2(teacher).to(device)
    # The generic parity checker accepts the shared TextNCA forward interface.
    ...
    (args.output_dir / f"r{args.replicate}-stage0.json").write_text(
        json.dumps({"format": "minicells.clm-v2-stage0.v1", **parity}, indent=2) + "\n"
    )
    student.freeze_inherited_backbone()
    optimizer = torch.optim.AdamW(student.sparse_parameters(), lr=1e-4,
                                  betas=(0.9, 0.95), weight_decay=0.1)
    total_sparse_steps = 2_000_000 // 1000
    ...
    if latest is not None:
        ...
        crossed_boundary = any(row["phase"] == "consolidation-k6" or
                               str(row["phase"]).startswith("top-") for row in rows)
        if crossed_boundary:
    ...
    imitation_pass = any(
        str(row["phase"]).startswith("imitation-") and float(row["quality_ratio"]) <= 1.05
        for row in rows
    )
    for attempt in range(2):
        ...
        trained = train_tokens(
            ...
            budget=500_000, seed=94001 + args.replicate * 100 + stage_index,
            device=device, local_only=True, local_weight=1.0,
        )
        dense_metrics = evaluate_current(student, validation, gate_starts, device)
        student.set_scaffold_alpha(0)
        counterfactual = evaluate_current(student, validation, gate_starts, device)
        student.set_scaffold_alpha(1)
        ratio = counterfactual["ppl"] / dense_metrics["ppl"]
        rows.append({"replicate": args.replicate, "phase": phase_name,
                     "alpha": 1.0, "top_k": 6, "quality_ratio": ratio,
                     **trained, **{f"validation_{k}": v for k, v in counterfactual.items()}})
        diagnostics.append({
            "replicate": args.replicate, "phase": phase_name,
            "router_grad_norm": trained["router_grad_norm"],
            **v2_router_diagnostics(student, validation, calibration_starts[:2],
                                    sequence_length=128, device=device),
        })
        save_v2_checkpoint(
            args.output_dir / f"r{args.replicate}-stage-{stage_index}.pt",
            checkpoint_payload(student, optimizer, scheduler, scaler,
                               args.replicate, stage_index, rows, diagnostics),
        )
        if ratio <= 1.05:
            imitation_pass = True
            break
    if not imitation_pass:
        status = "CLMV2_LOCAL_IMITATION_FAILURE"
    else:
        status = "RUNNING"
    ...
    if imitation_pass:
        for handoff in HANDOFF_STAGES:
            ...
            if existing is not None:
                if float(existing["quality_ratio"]) > 1.05:
                    status = "CLMV2_SCAFFOLD_HANDOFF_FAILURE"
                    break
                continue
            stage_index += 1
            ...
            trained = train_tokens(
                teacher, student, optimizer, scheduler, scaler, train,
                budget=handoff.tokens, seed=94001 + args.replicate * 100 + stage_index,
            ...
            rows.append({"replicate": args.replicate, "phase": handoff.name,
                         "alpha": handoff.alpha, "top_k": 6, "quality_ratio": ratio,
                         **trained, **{f"validation_{k}": v for k, v in metrics.items()}})
            diagnostics.append({
                "replicate": args.replicate, "phase": handoff.name,
                "router_grad_norm": trained["router_grad_norm"],
                **v2_router_diagnostics(student, validation, calibration_starts[:2],
                                        sequence_length=128, device=device),
            })
    ...
    if consolidation_row is not None:
        ...
        status = (
            ...
            if float(consolidation_row["quality_ratio"]) <= 1.03
            else "CLMV2_SCAFFOLD_HANDOFF_FAILURE"
        )
        ...
        metrics = evaluate_current(student, validation, gate_starts, device)
        teacher_nll = parity["teacher_nll"]
        teacher_ppl = math.exp(float(teacher_nll))
        ratio = metrics["ppl"] / teacher_ppl
        reference_k6_ppl = metrics["ppl"]
        rows.append({"replicate": args.replicate, "phase": "consolidation-k6",
                     "alpha": 0.0, "top_k": 6, "quality_ratio": ratio,
                     **trained, **{f"validation_{k}": v for k, v in metrics.items()}})
        diagnostics.append({
            "replicate": args.replicate, "phase": "consolidation-k6",
            "router_grad_norm": trained["router_grad_norm"],
            **v2_router_diagnostics(student, validation, calibration_starts[:2],
                                    sequence_length=128, device=device),
        })
        if ratio > 1.03:
            ...
            status = "CLMV2_SCAFFOLD_HANDOFF_SIGNAL"
        save_v2_checkpoint(
            args.output_dir / f"r{args.replicate}-stage-{stage_index}.pt",
            checkpoint_payload(student, optimizer, scheduler, scaler,
                               args.replicate, stage_index, rows, diagnostics, status),
        )

    if status == "CLMV2_SCAFFOLD_HANDOFF_SIGNAL":
        safe_state = {
        ...
        for top_k, budget in K_STAGES:
            ...
            if existing is not None:
                if float(existing["quality_ratio"]) > 1.03:
                    break
            ...
            trained = train_tokens(
                teacher, student, optimizer, scheduler, scaler, train,
                budget=budget, seed=94001 + args.replicate * 100 + stage_index,
                device=device, local_only=False, local_weight=0.25,
            )
            metrics = evaluate_current(student, validation, gate_starts, device)
            ratio = metrics["ppl"] / float(reference_k6_ppl)
            passed = ratio <= 1.03
            rows.append({"replicate": args.replicate, "phase": f"top-{top_k}",
                         "alpha": 0.0, "top_k": top_k, "quality_ratio": ratio,
                         **trained, **{f"validation_{k}": v for k, v in metrics.items()}})
            diagnostics.append({
                "replicate": args.replicate, "phase": f"top-{top_k}",
                "router_grad_norm": trained["router_grad_norm"],
                **v2_router_diagnostics(student, validation, calibration_starts[:2],
                                        sequence_length=128, device=device),
            })
    ...
    manifest = {"format": "minicells.clm-v2-validation-001-worker.v1", "complete": True,
                "replicate": args.replicate, "status": status,
                "quality_safe_k": quality_safe_k, "seed": seed,
                "stage_level_checkpoints": True, "cell_activation": 1.0}
