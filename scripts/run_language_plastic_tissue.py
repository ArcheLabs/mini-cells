...
def make_decision(
    ...
    return {
        ...
        "status": status,
        "question": (
            "Can homogeneous tissue cells within the existing three-stage CLM backbone, "
            "using only resource-constrained activity and a plastic reaction-diffusion connectome, "
            "produce stable, causally useful topology without expert-specific parameters, "
            "a router, hard top-k gate, or load-balancing loss?"
        ),
        "source_experiment": {
        ...
        "design": {
            ...
            "controlled_backbone": (
                "The existing three recurrent stage parameter sets and step embeddings are retained; "
                "homogeneity here means no cell/row/expert-specific learned rule within a stage."
            ),
            "router_parameters": 0,
            "top_k": False,
            "balance_loss": False,
...
def _draw_topology(ax, activity: np.ndarray, connectome: np.ndarray, title: str) -> None:
    ...
    for row, (x, y) in enumerate(xy):
        ax.text(x, y, "R0 / I-O" if row == 0 else f"R{row}", ha="center", va="center", fontsize=8)
    ax.set_title(title)
...
def save_plots(frames: dict[str, pd.DataFrame]) -> None:
    ...
    _draw_topology(ax, mean_activity, mean_connectome, "Experiment 015b — final plastic tissue topology\n(3 seeds × all validation tasks; top-2 incoming edges shown)")
    fig.tight_layout()
    ...
    for row in range(TISSUE_HEIGHT):
        ax.plot(mean_dynamics.index, mean_dynamics[f"activity_r{row}"], marker="o", label=f"R{row}")
    ...
    ax.plot(mean_dynamics.index, mean_dynamics["diffusion_to_reaction"], marker="o", label="diffusion / reaction")
    ax.set_xlabel("Evolution step")
...
def write_task_spec() -> None:
    payload = {
        ...
        "controlled_backbone": "existing three recurrent stage parameter sets retained",
        "mechanism": [
            "homogeneous cell rule within each recurrent stage",
...
def main() -> int:
    ...
    decision = make_decision(frames["model_summary"], ratios, frames["interventions"], gpu_count)
