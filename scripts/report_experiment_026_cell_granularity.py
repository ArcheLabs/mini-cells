...
def _plot_performance(trajectory: pd.DataFrame, output: Path) -> None:
    ...
    for granularity in GRANULARITIES:
        rows = trajectory[trajectory["granularity"] == granularity].sort_values("continuation_tokens")
        ax.plot(rows["continuation_tokens"] / 1e6, rows["balanced_nll"], marker="o", label=f"G={granularity}")
...
def _plot_differentiation(trajectory: pd.DataFrame, output: Path) -> None:
    ...
    for granularity in GRANULARITIES:
        ...
        ax.plot(
            ...
            rows["median_cell_specialization"],
            marker="o",
            label=f"G={granularity}",
        )
    ax.axhline(0.0, linewidth=1)
    ax.set_xlabel("Continuation tokens (M)")
    ax.set_ylabel("Median cell specialization (1 - normalized domain entropy)")
    ax.set_title("Experiment 026 — Functional differentiation")
    ax.grid(alpha=0.25)
    ax.legend()
    fig.tight_layout()
    fig.savefig(output, dpi=180)
...
def _plot_frontier(final: pd.DataFrame, output: Path) -> None:
    ...
    ax.scatter(final["balanced_nll"], final["median_cell_specialization"], s=70)
    for row in final.to_dict(orient="records"):
        ax.annotate(
            f"G={int(row['granularity'])}",
            (float(row["balanced_nll"]), float(row["median_cell_specialization"])),
            xytext=(6, 6),
            textcoords="offset points",
        )
    ax.set_xlabel("Final balanced four-domain NLL (lower is better)")
    ax.set_ylabel("Final median cell specialization (higher is more differentiated)")
...
def main() -> int:
    ...
    worker_summary = json.loads((results / "worker-summary.json").read_text(encoding="utf-8"))
    if worker_summary.get("complete") is not True:
        raise RuntimeError("refusing formal report: Experiment 026 workers are incomplete")
    protocol = json.loads((results / "protocol.json").read_text(encoding="utf-8"))
    if protocol.get("format") != FORMAT:
        raise RuntimeError(f"unexpected protocol format: {protocol.get('format')!r}")
    ...
    for granularity in GRANULARITIES:
        ...
        final = rows[rows["continuation_tokens"] == final_token].iloc[-1].to_dict()
    ...
    baseline_specialization = float(baseline["median_cell_specialization"])

    decision_rows = []
    for row in final.to_dict(orient="records"):
        granularity = int(row["granularity"])
        specialization_delta = float(row["median_cell_specialization"]) - baseline_specialization
        nll_ratio = float(row["balanced_nll"]) / max(baseline_nll, 1e-12)
        stability = float(row["mean_profile_stability_vs_age0"])
        qualifies = bool(
            granularity > 1
            and specialization_delta >= DIFFERENTIATION_MIN_DELTA
        ...
        decision_rows.append(
            {
                ...
                "specialization_delta_vs_g1": specialization_delta,
    ...
    qualifying.sort(
        key=lambda row: (
            -float(row["specialization_delta_vs_g1"]),
    ...
    age_zero_max = max(float(summaries[g]["age_zero_parity"]["max_abs_diff"]) for g in GRANULARITIES)
    ...
    decision = {
        ...
        "selected_granularity": int(selected["granularity"]) if selected is not None else None,
        ...
        "arms": decision_rows,
        "interpretation": (
            "A positive status means at least one finer granularity produced a preregistered increase in "
            "stable domain-selective micro-cell behavior without more than 2% balanced-NLL degradation. "
            "It does not establish autonomous mitosis or full NCA self-organization."
        ),
    }
    _json_write(results / "decision.json", decision)

    _plot_performance(trajectory, results / "performance-by-granularity.png")
    _plot_differentiation(trajectory, results / "differentiation-by-granularity.png")
    _plot_frontier(final, results / "granularity-frontier.png")

    print(json.dumps(decision, indent=2, sort_keys=True))
    if not parameter_parity:
        raise RuntimeError("Experiment 026 invalid: parameter count differs across granularity arms")
    if not age_zero_valid:
        raise RuntimeError("Experiment 026 invalid: age-zero function parity failed")
    if not math.isfinite(float(baseline_nll)):
