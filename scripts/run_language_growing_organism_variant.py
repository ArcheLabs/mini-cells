...
def make_schedule(stream_length: int, *, seed: int) -> tuple[tuple[int, ...], ...]:
    ...
    return tuple(tuple(int(v) for v in row) for row in values)
...
def evaluate_stream(model, token_stream: torch.Tensor, starts, *, variant: str, device: torch.device) -> dict[str, float]:
    ...
    for batch_starts in starts:
        ...
        losses = F.cross_entropy(logits.float().reshape(-1, logits.shape[-1]), targets.reshape(-1), reduction="sum")
...
def organism_snapshot(model, inputs: torch.Tensor, *, step: int, phase: str) -> tuple[list[dict], list[dict]]:
    ...
    activity = d.activity.float().mean(dim=(0, 1)).cpu()
    alive = d.alive_indices.cpu().tolist()
    cell_rows = []
...
def structural_probe_from_batch(model, inputs: torch.Tensor, targets: torch.Tensor):
    chunks = min(4, inputs.shape[0])
    input_parts = torch.chunk(inputs, chunks, dim=0)
    target_parts = torch.chunk(targets, chunks, dim=0)
    microbatches = [(x, y) for x, y in zip(input_parts, target_parts) if len(x)]
    return make_structural_probe(model, microbatches, loss_fn=ce_loss)
...
def evaluate_skill(model, corpus, *, device: torch.device) -> float:
    ...
    for start in range(0, len(corpus.sequences), 64):
        ...
        loss_sum += float(F.cross_entropy(selected_logits.reshape(-1, logits.shape[-1]), selected_targets.reshape(-1), reduction="sum"))
        tokens += selected_targets.numel()
    return loss_sum / max(1, tokens)


def local_skill_phase(
    model,
    ...
    *,
    ...
    for cell in ranked:
        ...
        if total <= 1e-12 or accumulated >= 0.80 * total:
            break
    newborn = [int(i) for i in range(model.max_cells) if bool(model.alive_mask[i]) and not bool(base_alive[i])]
    selected = sorted(set(selected + newborn))
    ...
    return rows, event_rows, transplant, localization
...
def main() -> int:
    ...
    for step in range(1, TRAIN_STEPS + 1):
        ...
        if step in CHECKPOINT_STEPS:
            ...
            checkpoint = {
                ...
                "seconds_per_million_tokens": elapsed / ((step * TOKENS_PER_STEP) / 1_000_000),
                "grad_norm": grad_norm,
                "learning_rate": lr,
    ...
    if variant == "G":
        ...
        local_rows, local_events, transplant_rows, localization_rows = local_skill_phase(
            model,
            base_state,
            validation_stream,
            validation_starts,
        ...
        structural_rows.extend({"replicate": replicate, **row} if "replicate" not in row else row for row in local_events)
    ...
    worker = {
        ...
        "final_alive_cells": model.alive_count if variant != "T" else None,
        "final_edges": model.edge_count if variant != "T" else None,
