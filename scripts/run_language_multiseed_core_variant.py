...
from minicells.language_depth_ablation import resolve_stage_depths, variant_by_code  # noqa: E402
...
from minicells.language_stabilization import make_depth_schedule, stabilizing_forward  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run one Experiment 014 matched multiseed cell.")
    parser.add_argument("--topology", choices=("1d", "2d"), required=True)
    parser.add_argument("--variant", choices=CORE_VARIANT_CODES, required=True)
...
def build_model(topology: str, vocab_size: int, replicate: int) -> tuple[torch.nn.Module, int, dict[str, object]]:
    seeds = seed_bundle(replicate)
    seed = model_seed(seeds, topology)
    torch.manual_seed(seed)
    if topology == "1d":
        model = build_minicells_v2(vocab_size)
        metadata: dict[str, object] = {
            "architecture": "1D MiniCells-v2",
            "seed": seed,
        }
    elif topology == "2d":
        model = build_minicells_2d(vocab_size, tissue_height=4)
        metadata = {
            "architecture": "2D MiniCells K=4",
            "seed": seed,
            "tissue_height": 4,
        }
    else:
    ...
    metadata["step_embedding_init_scale"] = 1.0
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("Experiment 014 worker requires CUDA")
    ...
    if variant.code not in CORE_VARIANT_CODES or variant.step_embedding_init_scale != 1.0:
        raise RuntimeError("Experiment 014 only accepts the scale-1.0 A/B/F/H core variants")
    seeds = seed_bundle(args.replicate)
    run_name = f"{args.topology}-r{args.replicate}-{variant.code}"
    ...
    if not tokenizer_path.is_file() or not train_path.is_file() or not validation_path.is_file():
        raise FileNotFoundError("Experiment 014 cache is incomplete")
    ...
    validation_starts = fixed_validation_starts(
        ...
        batch_size=8,
        sequence_length=128,
        seed=seeds.validation_seed,
    )
    depth_schedule = make_depth_schedule(schedule.steps, seed=seeds.depth_seed)

    model, seed, model_metadata = build_model(args.topology, tokenizer.get_vocab_size(), args.replicate)
    ...
    print(
        {
            ...
            "seed_bundle": seeds.as_dict(),
        }
    )

    # Re-seed after construction so all A/B/F/H cells in a matched topology
    # replicate see the same stochastic training stream from the same weights.
    torch.manual_seed(seed)
    ...
    for step, starts in enumerate(schedule.starts, start=1):
        ...
        if consumed in checkpoint_set:
            ...
            rows.append(row)
            print(
                f"{run_name:9s} tokens={consumed:9d} ppl={row['validation_ppl']:.2f} "
    ...
    model.eval()
    depth_frame = evaluate_depth_robustness(
        model,
        validation_stream,
        device=device,
        eval_seed=seeds.depth_eval_seed,
    )
    depth_frame.insert(0, "run", run_name)
    depth_frame.insert(1, "replicate", args.replicate)
    depth_frame.insert(2, "topology", args.topology)
    depth_frame.insert(3, "variant", variant.code)
    ...
    worker = {
        "format": "minicells.language-multiseed-core-worker.v1",
        "run": run_name,
        "replicate": args.replicate,
        "topology": args.topology,
        "variant": variant.code,
