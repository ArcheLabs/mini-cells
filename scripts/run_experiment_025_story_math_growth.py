#!/usr/bin/env python3
"""Orchestrate Experiment 025 on up to two CUDA GPUs.

The orchestrator is designed for Kaggle "Save Version -> Run All" execution.  It
uses a global wall-clock budget, runs incomplete arms concurrently, and resumes
from periodic checkpoints in additional rounds while enough budget remains.
"""

from __future__ import annotations

import argparse
import json
import os
import shutil
import subprocess
import sys
import time
from pathlib import Path

import torch

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "research"))

from minicells.language_30m import prepare_30m_corpus  # noqa: E402
from minicells.language_data import load_tokenizer  # noqa: E402
from minicells.story_math_shift_30m import (  # noqa: E402
    RESULT_DIR_NAME,
    SHIFT_TOKENS,
    experiment_budget,
    prepare_math_corpus,
    schedule_manifest,
)

SOURCE_006 = ROOT / "artifacts" / "experiments" / "006-consumer-language-scaling"
OUT = ROOT / "results" / RESULT_DIR_NAME
WORKER = ROOT / "scripts" / "run_experiment_025_story_math_worker.py"
REPORT = ROOT / "scripts" / "report_experiment_025_story_math_growth.py"

DEFAULT_TOTAL_WALL_HOURS = 8.0
DEFAULT_FINALIZATION_RESERVE_MINUTES = 30.0
DEFAULT_ROUND_WALL_HOURS = 2.5
MIN_RESUME_WINDOW_MINUTES = 20.0


def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(description="Run Experiment 025 — Story→Math growth")
    result.add_argument("--shift-tokens", type=int, default=SHIFT_TOKENS)
    result.add_argument(
        "--total-wall-hours",
        type=float,
        default=DEFAULT_TOTAL_WALL_HOURS,
        help="Whole Kaggle job budget, including setup, training, reporting and publication reserve.",
    )
    result.add_argument(
        "--finalization-reserve-minutes",
        type=float,
        default=DEFAULT_FINALIZATION_RESERVE_MINUTES,
        help="Budget held back for final evaluation/reporting after workers stop.",
    )
    result.add_argument(
        "--round-wall-hours",
        type=float,
        default=DEFAULT_ROUND_WALL_HOURS,
        help="Maximum wall time given to one worker invocation before checkpoint/resume.",
    )
    result.add_argument("--reset", action="store_true")
    return result.parse_args()


def _git(*args: str) -> str:
    return subprocess.check_output(["git", *args], cwd=ROOT, text=True).strip()


def _json_write(path: Path, value: object) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(value, indent=2, sort_keys=True) + "\n", encoding="utf-8")


def _read_worker_summary(arm: str) -> dict[str, object] | None:
    path = OUT / arm / "worker-summary.json"
    if not path.is_file():
        return None
    return json.loads(path.read_text(encoding="utf-8"))


def _completed_arms() -> set[str]:
    result: set[str] = set()
    for arm in ("llm", "clm"):
        summary = _read_worker_summary(arm)
        if summary is not None and bool(summary.get("complete")):
            result.add(arm)
    return result


def worker_command(
    arm: str,
    *,
    story_cache: Path,
    math_cache: Path,
    shift_tokens: int,
    max_wall_hours: float,
) -> list[str]:
    return [
        sys.executable,
        str(WORKER),
        "--arm",
        arm,
        "--story-cache-dir",
        str(story_cache),
        "--math-cache-dir",
        str(math_cache),
        "--output-dir",
        str(OUT),
        "--shift-tokens",
        str(shift_tokens),
        "--max-wall-hours",
        str(max_wall_hours),
    ]


def _run_round(
    arms: tuple[str, ...],
    *,
    used_gpus: int,
    story_cache: Path,
    math_cache: Path,
    shift_tokens: int,
    round_wall_hours: float,
    round_index: int,
) -> None:
    if not arms:
        return
    if used_gpus == 1:
        for arm in arms:
            env = os.environ.copy()
            env["CUDA_VISIBLE_DEVICES"] = "0"
            log_path = OUT / f"{arm}.round-{round_index}.log"
            with log_path.open("w", encoding="utf-8") as handle:
                result = subprocess.run(
                    worker_command(
                        arm,
                        story_cache=story_cache,
                        math_cache=math_cache,
                        shift_tokens=shift_tokens,
                        max_wall_hours=round_wall_hours,
                    ),
                    cwd=ROOT,
                    env=env,
                    stdout=handle,
                    stderr=subprocess.STDOUT,
                    text=True,
                    check=False,
                )
            print(f"--- {arm} / GPU 0 / round {round_index} ---")
            print(log_path.read_text(encoding="utf-8").rstrip())
            if result.returncode != 0:
                raise RuntimeError(f"{arm} exited {result.returncode}; see {log_path}")
        return

    active: list[tuple[str, int, subprocess.Popen[str], Path]] = []
    for gpu_index, arm in enumerate(arms):
        env = os.environ.copy()
        env["CUDA_VISIBLE_DEVICES"] = str(gpu_index)
        log_path = OUT / f"{arm}.round-{round_index}.log"
        handle = log_path.open("w", encoding="utf-8")
        process = subprocess.Popen(
            worker_command(
                arm,
                story_cache=story_cache,
                math_cache=math_cache,
                shift_tokens=shift_tokens,
                max_wall_hours=round_wall_hours,
            ),
            cwd=ROOT,
            env=env,
            stdout=handle,
            stderr=subprocess.STDOUT,
            text=True,
        )
        handle.close()
        active.append((arm, gpu_index, process, log_path))
        print(f"started {arm:3s} on physical GPU {gpu_index} / round {round_index}")

    failures: list[str] = []
    for arm, gpu_index, process, log_path in active:
        code = process.wait()
        print(f"--- {arm} / GPU {gpu_index} / round {round_index} ---")
        print(log_path.read_text(encoding="utf-8").rstrip())
        if code != 0:
            failures.append(f"{arm} exited {code}; see {log_path}")
    if failures:
        raise RuntimeError("; ".join(failures))


def main() -> int:
    args = parser().parse_args()
    if args.shift_tokens <= 0 or args.shift_tokens > SHIFT_TOKENS:
        raise ValueError("--shift-tokens must be in (0, 50M]")
    if args.total_wall_hours <= 0 or args.total_wall_hours > 8.0:
        raise ValueError("--total-wall-hours must be in (0, 8]")
    if args.finalization_reserve_minutes < 10:
        raise ValueError("reserve at least 10 minutes for finalization")
    if args.round_wall_hours <= 0:
        raise ValueError("--round-wall-hours must be positive")

    job_started = time.monotonic()
    job_deadline = job_started + args.total_wall_hours * 3600.0
    finalization_reserve = args.finalization_reserve_minutes * 60.0
    training_deadline = job_deadline - finalization_reserve
    if training_deadline <= job_started:
        raise ValueError("finalization reserve consumes the whole job budget")

    dirty = subprocess.check_output(
        ["git", "status", "--porcelain", "--untracked-files=no"],
        cwd=ROOT,
        text=True,
    ).strip()
    if dirty:
        raise RuntimeError("Experiment 025 requires a clean tracked Git tree")

    available = torch.cuda.device_count()
    if available < 1:
        raise RuntimeError("Experiment 025 requires at least one CUDA GPU")
    used = min(2, available)
    OUT.mkdir(parents=True, exist_ok=True)
    if args.reset:
        # Preserve expensive corpus caches and the reproducible LLM pretrain if present.
        for arm in ("llm", "clm"):
            arm_dir = OUT / arm
            if not arm_dir.exists():
                continue
            for path in arm_dir.iterdir():
                if arm == "llm" and path.name == "pretrain":
                    continue
                if path.is_dir():
                    shutil.rmtree(path)
                else:
                    path.unlink()

    print("preparing/reusing Experiment-007 TinyStories corpus")
    corpus = prepare_30m_corpus(ROOT, source_006_dir=SOURCE_006)
    story_cache = corpus.train_path.parent
    tokenizer = load_tokenizer(corpus.tokenizer_path)
    print("preparing/reusing synthetic arithmetic corpus")
    math_cache = OUT / "cache"
    arithmetic = prepare_math_corpus(math_cache, tokenizer)

    provenance = {
        "format": "minicells.story-math-shift-30m-run.v2",
        "code_commit": _git("rev-parse", "HEAD"),
        "code_tree_sha": _git("rev-parse", "HEAD^{tree}"),
        "tracked_tree_dirty": False,
        "gpu_count_visible": available,
        "gpu_names": [torch.cuda.get_device_name(index) for index in range(available)],
        "gpu_count_used": used,
        "budget": {
            **experiment_budget(),
            "global_wall_hours": args.total_wall_hours,
            "finalization_reserve_minutes": args.finalization_reserve_minutes,
            "worker_round_wall_hours": args.round_wall_hours,
            "one_shot_kaggle_run_all": True,
        },
        "requested_shift_tokens": args.shift_tokens,
        "schedule": schedule_manifest(args.shift_tokens),
        "story_corpus_manifest": corpus.manifest,
        "math_corpus_manifest": arithmetic["manifest"],
    }
    _json_write(OUT / "run-provenance.json", provenance)

    round_index = 0
    while True:
        completed = _completed_arms()
        incomplete = tuple(arm for arm in ("llm", "clm") if arm not in completed)
        if not incomplete:
            break
        remaining_training_seconds = training_deadline - time.monotonic()
        if remaining_training_seconds < MIN_RESUME_WINDOW_MINUTES * 60.0:
            print(
                "stopping automatic resume: remaining training budget is below "
                f"{MIN_RESUME_WINDOW_MINUTES:.0f} minutes"
            )
            break
        round_index += 1
        round_hours = min(
            args.round_wall_hours,
            remaining_training_seconds / 3600.0,
        )
        print(
            f"automatic round {round_index}: incomplete={list(incomplete)}, "
            f"worker_budget={round_hours:.2f}h, "
            f"global_remaining={(job_deadline - time.monotonic())/3600.0:.2f}h"
        )
        _run_round(
            incomplete,
            used_gpus=used,
            story_cache=story_cache,
            math_cache=math_cache,
            shift_tokens=args.shift_tokens,
            round_wall_hours=round_hours,
            round_index=round_index,
        )

    summaries: dict[str, dict[str, object]] = {}
    for arm in ("llm", "clm"):
        summary = _read_worker_summary(arm)
        if summary is None:
            summary = {
                "arm": arm,
                "complete": False,
                "reason": "worker did not emit a summary before global deadline",
            }
        summaries[arm] = summary
    complete = all(bool(summaries[arm].get("complete")) for arm in ("llm", "clm"))
    _json_write(
        OUT / "worker-summary.json",
        {
            "format": "minicells.story-math-shift-30m-workers.v2",
            "complete": complete,
            "automatic_rounds": round_index,
            "global_elapsed_seconds": time.monotonic() - job_started,
            "global_wall_hours": args.total_wall_hours,
            "arms": summaries,
        },
    )

    if complete:
        subprocess.run(
            [sys.executable, str(REPORT), "--results-dir", str(OUT)],
            cwd=ROOT,
            check=True,
        )
        print(f"Experiment 025 complete: {OUT}")
    else:
        print(
            "Experiment 025 did not complete inside the global 8h budget. "
            "All available worker checkpoints and partial metrics were preserved."
        )
    print(
        f"job elapsed={(time.monotonic() - job_started)/3600.0:.2f}h; "
        f"remaining={(job_deadline - time.monotonic())/3600.0:.2f}h"
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
